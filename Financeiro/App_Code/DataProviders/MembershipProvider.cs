...
namespace PROJETO
{
	...
	public class MembershipTokenProvider : IGeneralDataProvider
    {

        public LoginTokenItem CurrentItem;
        ...
        public string TableName
        {
            get { return GMembershipProvider.Default.TokenTableName; }
        }
        ...
        private LoginTokenDataProvider LoginTokenProvider;
        public GeneralDataProvider DataProvider
        {
            get { return LoginTokenProvider; }
            set { LoginTokenProvider = value as LoginTokenDataProvider; }
        }
        ...
        public void SetParametersValues(GeneralDataProvider Provider)
        {
            DataProvider.Parameters["Token"].Parameter.SetValue(CurrentItem.Fields["Token"].Value);
        }
        ...
        public MembershipTokenProvider()
        {
            LoginTokenProvider = new LoginTokenDataProvider(this, TableName, DatabaseName, "", "");
        }

        internal bool SelectToken(string Token)
        {
            CurrentItem = new LoginTokenItem();
            CurrentItem.Fields["Token"].SetValue(Token);
            CurrentItem = DataProvider.SelectItem(0, FormPositioningEnum.First) as LoginTokenItem;
            return (CurrentItem != null);
        }
	...
	public class GMembershipProvider : MembershipProvider
	{
		...
		private MembershipTokenProvider _TokenProvider;
        public MembershipTokenProvider TokenProvider
        {
            get
            {
                if (_TokenProvider == null)
                {
                    _TokenProvider = new MembershipTokenProvider();
                }
        ...
        private string _TokenTableName;
        private string _TokenUserIdField;
        private string _TokenIdField;
        private string _TokenAcquiredField;
        private string _TokenExpirationField;
        ...
        public string TokenTableName { get { return _TokenTableName; } set { _TokenTableName = value; } }
        public string TokenUserIdField { get { return _TokenUserIdField; } set { _TokenUserIdField = value; } }
        public string TokenIdField { get { return _TokenIdField; } set { _TokenIdField = value; } }
        public string TokenAcquiredField { get { return _TokenAcquiredField; } set { _TokenAcquiredField = value; } }
        public string TokenExpirationField { get { return _TokenExpirationField; } set { _TokenExpirationField = value; } }
		...
		public override void Initialize(string Name, System.Collections.Specialized.NameValueCollection Config)
		{
            ...
            TokenTableName = Config["TokenTableName"];
            TokenIdField = Config["TokenIdField"];
            TokenUserIdField = Config["TokenUserIdField"];
            TokenAcquiredField = Config["TokenAcquiredField"];
            TokenExpirationField = Config["TokenExpirationField"];
		...
		public LoginTokenItem GetToken(string username, string password)
        {
            if (UserProvider.ValidateUser(username, password))
            {
                LoginTokenItem item = new LoginTokenItem();
                item.SetDefaultToken();
                item.Fields["Token"].SetValue(Guid.NewGuid().ToString());
                item.Fields["User"].SetValue(Crypt.Encripta(username));
                TokenProvider.DataProvider.InsertItem(item);
                return item;
            }
            return null;
        }

        public LoginTokenItem CheckToken(string token)
        {
            if (TokenProvider.SelectToken(token))
            {
                return TokenProvider.CurrentItem;
            }
            return null;
        }
