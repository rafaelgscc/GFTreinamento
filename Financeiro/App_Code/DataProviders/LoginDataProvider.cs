...
Fields["IsAdmin"].SetValue(true);
	...
	public class LoginTokenItem : GeneralDataProviderItem
    {

        public LoginTokenItem()
        {
            Fields.Add("Token", new TextField(GMembershipProvider.Default.TokenIdField, "", null));
            Fields.Add("User", new TextField(GMembershipProvider.Default.TokenUserIdField, "", null));
            Fields.Add("Acquired", new DateField(GMembershipProvider.Default.TokenAcquiredField, "", null));
            Fields.Add("Expiration", new DateField(GMembershipProvider.Default.TokenExpirationField, "", null));
        }

        public void SetDefaultToken()
        {
            Fields["Acquired"].SetValue(DateTime.Now);
            Fields["Expiration"].SetValue(DateTime.Now.AddYears(100));
        }

    }

    public class LoginTokenDataProvider : GeneralDataProvider
    {

        public LoginTokenDataProvider(IGeneralDataProvider BasePage, string TableName, string DataBaseName, string IndexName, string Name) : base(BasePage, TableName, DataBaseName, IndexName, Name)
        {
        }
        ...
        {
            Parameters.Clear();
            Parameters.Add("Token", new ParameterStruct(GMembershipProvider.Default.TokenIdField, new TextParameter(), true, true, true, false));
            base.CreateParameters();
        }
    }
