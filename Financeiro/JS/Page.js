...
function CheckValidation(sender, eventArgs) 
{
	var commandName = eventArgs.get_commandName();
	if (commandName == "Update" || commandName == "PerformInsert") 
	{
		if (!jQuery(sender._element).validationEngine("validate")) eventArgs.set_cancel(true);
	}
	else if (commandName == "CancelUpdate" || commandName == "CancelInsert") 
	{
		jQuery(sender._element).validationEngine("hideAll");
	}
}
