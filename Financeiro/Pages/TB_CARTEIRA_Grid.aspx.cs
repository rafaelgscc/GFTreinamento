...
{
	public partial class TB_CARTEIRA_Grid : GeneralDataPage
	{
		protected TB_CARTEIRA_GridPageProvider PageProvider;
		...
		public override string PageName { get { return "TB_CARTEIRA_Grid.aspx"; } }
		...
		{
			PageProvider = new TB_CARTEIRA_GridPageProvider(this);
		}
		...
		{
			Grid_TB_CARTEIRA.CurrentPageIndex = 0;
			Grid_TB_CARTEIRA.DataSource = null;
			Grid_TB_CARTEIRA.Rebind();
		}


		protected void Grid_TB_CARTEIRA_PreRender(object sender, EventArgs e)
		{
			if (Grid_TB_CARTEIRA.MasterTableView.Items.Count == 0)
			{
				if (Grid_TB_CARTEIRA.MasterTableView.IsItemInserted == false)
				{
					Grid_TB_CARTEIRA.ShowHeader = false;
				}
				else
				{
					Grid_TB_CARTEIRA.ShowHeader = true;
				}
			}
			else
			{
		...
		public override GeneralDataProviderItem LoadItemFromGridControl(bool EnableValidation, string GridId)
		{
			...
			switch (GridId)
			{
				case "Grid_TB_CARTEIRA":
					if (PageProvider.TB_CARTEIRA_Grid_Grid_TB_CARTEIRAProvider.DataProvider.Item == null)
						Item = PageProvider.TB_CARTEIRA_Grid_Grid_TB_CARTEIRAProvider.GetDataProviderItem();
					else
						Item = PageProvider.TB_CARTEIRA_Grid_Grid_TB_CARTEIRAProvider.DataProvider.Item;
					PageProvider.TB_CARTEIRA_Grid_Grid_TB_CARTEIRAProvider.RaiseSetRelationFields(PageProvider.TB_CARTEIRA_Grid_Grid_TB_CARTEIRAProvider, Item);
					Item.SetFieldValue(Item["CAR_NOME"], PageProvider.TB_CARTEIRA_Grid_Grid_TB_CARTEIRAProvider.GridData["CAR_NOME"]);
					PageProvider.TB_CARTEIRA_Grid_Grid_TB_CARTEIRAProvider.InitializeAlias(Item);
					if (EnableValidation)
					{
						PageProvider.TB_CARTEIRA_Grid_Grid_TB_CARTEIRAProvider.Validate(Item);
					}
					break;
			}
		...
		public override void setGridPerm()
		{
			...
			{
				Grid_TB_CARTEIRA.MasterTableView.CommandItemDisplay = GridCommandItemDisplay.None;
			}
			if (Grid_TB_CARTEIRA.Columns[0] is GridEditCommandColumn && !PageOperations.AllowUpdate)
			{
				Grid_TB_CARTEIRA.Columns[0].Visible = false;
			}
			if (Grid_TB_CARTEIRA.Columns.Count != 0 && Grid_TB_CARTEIRA.Columns[Grid_TB_CARTEIRA.Columns.Count - 1] is GridButtonColumn && (Grid_TB_CARTEIRA.Columns[Grid_TB_CARTEIRA.Columns.Count - 1] as GridButtonColumn).CommandName == "Delete" && !PageOperations.AllowDelete)
			{
				Grid_TB_CARTEIRA.Columns[Grid_TB_CARTEIRA.Columns.Count - 1].Visible = false;
			}
		}

		protected void Grid_TB_CARTEIRA_ItemCreated(object sender, GridItemEventArgs e)
		{
			if (e.Item is GridEditableItem && (e.Item.IsInEditMode))
			{
				if (Grid_TB_CARTEIRA.Columns[0].ColumnType == "GridEditCommandColumn" && PageOperations.AllowUpdate)
				{
					if (Grid_TB_CARTEIRA.Columns[0].HeaderStyle.Width == 20 && Grid_TB_CARTEIRA.Columns[0].Visible == true)
					{
						Grid_TB_CARTEIRA.Columns[0].HeaderStyle.Width = 70; 
					}
					else
					{
						Grid_TB_CARTEIRA.Columns[0].HeaderStyle.Width = 20; 
					}
				}
				GridEditableItem editableItem = (GridEditableItem)e.Item;
				TextBox txt;
				txt = (editableItem.EditManager.GetColumnEditor("GridColumn_CAR_NOME") as GridTextBoxColumnEditor).TextBoxControl;
				txt.Width = 78;
				txt.Attributes.Add("data-validation-engine", "validate[funcCall[GridColumn_CAR_NOME_Validation]]");
				txt.Attributes.Add("data-validation-message", "Nome da Carteira não pode ser vazio!");
				AjaxPanel.ResponseScripts.Add("jQuery(\"#Grid_TB_CARTEIRA\").validationEngine();");
		...
		protected void Grid_TB_CARTEIRA_ItemDataBound(object sender, GridItemEventArgs e)
		{
		...
		protected void Grid_TB_CARTEIRA_ItemCommand(object source, GridCommandEventArgs e)
		{
			...
			if (e.Item is GridDataItem && !(e.Item is GridDataInsertItem) && e.CommandName != "Cancel" && e.CommandName != "Update" && e.CommandName != "Insert")
			{
				...
				switch (e.CommandName)
				{
					case "GridColumn2":
						...
						try
						{
							string UrlPage = ResolveUrl("~/carteira");
							UrlPage += "/editar";
							UrlPage += "/" + (Convert.ToInt32(Provider.DataProvider.Item["CAR_ID"].GetValue())).ToString();
							try
							{
								if (!IsPostBack)
								{
		...
		{
			Grid_TB_CARTEIRA.MasterTableView.ClearEditItems();
			Grid_TB_CARTEIRA.MasterTableView.IsItemInserted = false;
		}
		...
		public override GeneralGridProvider GetGridProvider(RadGrid Grid)
		{
			...
			{
				case "Grid_TB_CARTEIRA":
					return PageProvider.TB_CARTEIRA_Grid_Grid_TB_CARTEIRAProvider;
			}
			return null;
		}
	}
